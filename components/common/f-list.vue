<template>
	<view>
		<view class="position-relative border-primary" style="width: 365rpx; height: 365rpx;" @click="openLive">
			<!-- 图片 -->
			<image :src="item.cover" class="position-absolute" mode="aspectFill" style="width: 100%; height: 100%;"></image>
			<!-- 钱 -->
			...
			<view class="rounded-circle px-2 flex align-center" style="position: absolute; right: 15rpx; top: 15rpx; background-color: rgba(0,0,0,0.4);">
				...
				<text class="text-white font">{{ item.hot }}</text>
			</view>

			<!-- 标题 -->
			<view class="rounded-circle px-2 flex align-center" style="position: absolute; left: 15rpx; bottom: 15rpx; background-color: rgba(0,0,0,0.4);">
			...
			<view class="rounded-circle px-2 flex align-center" style="position: absolute; right: 15rpx; bottom: 15rpx; background-color: rgba(0,0,0,0.4);">
				<text style="width: 20rpx; height: 20rpx;" :class="iconClass" class="rounded-circle mr-1"></text>
				<text class="text-white font-sm">{{ item.status === true ? '直播中' : '已结束' }}</text>
			</view>
		</view>
	</view>
</template>
...
<script>
	export default {
		...
		computed: {
			iconClass() {
				let color = this.item.status === true ? 'bg-success' : 'bg-danger';
				return color;
			}
		}
	};
